...
if (isset($_POST['action']) && $_POST['action'] == 'filter') {
    $sql = "SELECT * FROM timeOut WHERE 1=1"; // Use your 'timeOut' table name
    ...
    if (!empty($timeOut)) {
        $sql .= " AND timeOut = '$timeOut'";
    }
    if (!empty($date)) {
        $sql .= " AND DATE(timeOut) = '$date'"; // Filtering by date
    }
    if (!empty($searchTerm)) {
    ...
    $countQuery = "SELECT COUNT(*) as total FROM timeOut WHERE 1=1";
    if (!empty($department)) {
    ...
    if (!empty($timeOut)) {
        $countQuery .= " AND timeOut = '$timeOut'";
    }
    if (!empty($date)) {
        $countQuery .= " AND DATE(timeOut) = '$date'"; // Filtering by date
    }
    if (!empty($searchTerm)) {
  ...
  <table border="1">
    <thead>
      <tr>
        ...
        <th>Year</th>
        <th>Section</th>
        <th>Time Out <button onclick="sortTable('timeOut')"><i class='fas fa-sort'></i></button></th>
      </tr>
    </thead>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#department, #year, #section, #timeOut, #date, #search, #limit').on('input change', function() {
            filterAttendance();
        });
